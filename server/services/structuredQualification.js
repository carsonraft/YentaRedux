...
class StructuredQualificationService {
  ...
  async startQualification(prospectId, companyName = '') {
    try {
      ...
      const conversationResult = await db.query(
        `INSERT INTO prospect_conversations (prospect_id, conversation_type, status)
  ...
  async processResponse(conversationId, userResponse) {
    try {
      ...
      // Update qualification record with new data
      const updatedData = { ...qualState.extracted_data, ...extractedData };
  ...
  async extractDataFromResponse(userResponse, targetFields, existingData = {}) {
    const extractionPrompt = `Extract structured data from this user response for a business AI qualification.
...
Extract only the target fields listed above. Return a JSON object with the extracted values. Use these specific categories:
...
budgetAmount: extract any specific dollar amounts mentioned
